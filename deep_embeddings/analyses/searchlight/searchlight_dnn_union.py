...
def parseargs():
    ...
    aa('--in_path_imagenet', type=str, default='./ILSVRC/Data/CLS-LOC/val/',
        help='path to ImageNet validation set')
    aa('--in_path_things', type=str, default='./THINGS_full',
        help='path to THINGS images (whole dataset)')
    aa('--out_path_imagenet', type=str, default='./ILSVRC/features/val_set/vgg16_bn/classifier.3/',
        help='path to DNN features for each image in the ImageNet validation set')
    aa('--out_path_things', type=str, default='./activations/vgg16_bn/classifier.3/THINGS/full/',
        help='path to DNN features for each image in the THINGS database (whole dataset)')
...
def load_features(out_path_imagenet:str, out_path_things:str) -> np.ndarray:
    with open(pjoin(out_path_imagenet, 'features.npy'), 'rb') as f:
        imagenet_features = np.load(f)
    with open(pjoin(out_path_things, 'features.npy'), 'rb') as f:
        things_features = np.load(f)
    features = np.vstack((imagenet_features, things_features))
    return features

def get_file_names(out_path_imagenet:str, out_path_things:str) -> List[str]:
    file_names_imagenet = open(pjoin(out_path_imagenet, 'file_names.txt'), 'r').read().splitlines()
    file_names_things = open(pjoin(out_path_things, 'file_names.txt'), 'r').read().splitlines()
    file_names = file_names_imagenet + file_names_things
    return file_names
...
def search_img_space(
                     model,
                     in_path_imagenet:str,
                     in_path_things:str,
                     out_path_imagenet:str,
                     out_path_things:str,
                     regression_path:str,
                     model_name:str,
                     module_name:str,
    ...
    dataset_imagenet = ImageDataset(root=in_path_imagenet, out_path='', things=False, things_behavior=False, add_ref_imgs=False, transforms=transforms)
    dataset_things = ImageDataset(root=in_path_things, out_path='', things=True, things_behavior=False, add_ref_imgs=True, transforms=transforms)
    dataset = tuple(dataset_imagenet) + tuple(dataset_things)

    predictor = joblib.load(pjoin(regression_path, f'predictor_{latent_dim:02d}.joblib'))
    file_names = get_file_names(out_path_imagenet, out_path_things)
    features = load_features(out_path_imagenet, out_path_things)
    ...
    out_path = pjoin('searchlights', 'dnn', 'union', analysis, 'top_k', f'{latent_dim:02d}')
...
if __name__ == '__main__':
    ...
    search_img_space(
                    model=model,
                    in_path_imagenet=args.in_path_imagenet,
                    in_path_things=args.in_path_things,
                    out_path_imagenet=args.out_path_imagenet,
                    out_path_things=args.out_path_things,
