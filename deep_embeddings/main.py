...
parser.add_argument("--beta", type=float, default=1.0, help="Beta parameter to balance KL div and reconstruction loss")
...
def parse_number_of_objects(triplet_path, modality):
    if modality == "behavior":
        return 1854
    else:
        base_path = os.path.dirname(os.path.dirname(triplet_path))
...
def train(triplet_path, log_path, modality, fresh=False, load_model=False, tensorboard=False, 
    ...
    n_objects = parse_number_of_objects(triplet_path, modality)
    ...
    if prior == "gauss":
        ...
        raise ValueError("Prior must be either 'gauss' or 'sslab'")
    ...
    if modality == "deep":
        log_path = os.path.join(log_path, identifier, modality, n_mio_samples, prior, str(init_dim), str(batch_size),
                                str(beta), str(scale), str(rnd_seed))

    else:
        model_name, module_name = triplet_path.split("/")[-2:]
        log_path = os.path.join(log_path, identifier, modality, model_name, module_name, n_mio_samples, prior, str(init_dim), str(batch_size),
                                str(beta), str(scale), str(rnd_seed))
