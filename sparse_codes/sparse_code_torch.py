...
class ElasticNet(nn.Module):
    ...
    def get_loss(self, x, y):
        """ Combined loss function for Elastic Net with L1 and L2 penalty. Same as sklearn loss function. """
        y_pred = self(x)
        loss = 1 / (2 * x.shape[0]) * torch.sum((y_pred - y) ** 2) 
        return loss + self.elastic_penalty()
...
def save_regression_predictions(weight, bias, path):
    ...
    out_path = os.path.join(path, 'regression_predictions')
...
def train():
    ...
    # dnn_path = '/home/florian/THINGS/vgg_bn_features12/features.npy'
    # embedding_path = "../learned_embeddings/weights_triplets_50mio/params/pruned_q_mu_epoch_500.txt"
    ...
    # embedding_path = "../learned_embeddings/weights_things_behavior_256bs/params/pruned_q_mu_epoch_3000.txt"
    ...
    for k, (train_idx, test_idx) in enumerate(kf.split(X)):
        ...
        # NOTE we dont want to do gradient descent, but coordinate descent!
        ...
        for iter in range(max_iter):
            ...
            with torch.no_grad():
                ...
                if val_loss < best_val_loss:
                    ...
                    # r2_train = r2_score(y_train.detach().cpu().numpy(), y_pred_train.detach().cpu().numpy())
