...
parser.add_argument(
    "--tripletize", action="store_true", default=False, help="Tripletize the features"
)
parser.add_argument(
    "--adaptive", action="store_true", default=False, help="Adaptively sample triplets"
)
parser.add_argument(
    "--n_samples",
    type=int,
...
def run_pipeline(
    ...
    if extract:
        print(
            "Extracting features from model: {} and module: {}".format(
    ...
    if tripletize:
        ...
        sampler = Sampler(
            in_path, out_path, n_samples=n_samples, k=3, train_fraction=0.9, seed=seed
        )
