...
def plot_causal_comparison(embedding_path, dnn_path, img_root):
    ...
    X = load_deepnet_activations(
        dnn_path, center=True, zscore=False, relu=True, to_torch=True
    )
    ...
    for fname in FNAMES:
        ...
        for color, index_top, index_bottom in zip(
            ["red", "green", "blue"], top_3_top, top_3_bottom
        ):
            ...
            ax.annotate(
                index_top,
                xy=(index_top, diff_predictions[index_top]),
                xytext=(index_top, diff_predictions[index_top] + 0.1),
                arrowprops=dict(
                    facecolor=color, shrink=0.05, width=0.5, headwidth=3, headlength=5
                ),
                rotation=45,
            )
            ...
            ax.annotate(
                index_bottom,
                xy=(index_bottom, diff_predictions[index_bottom]),
                xytext=(index_bottom, diff_predictions[index_bottom] + 0.1),
                arrowprops=dict(
                    facecolor=color, shrink=0.05, width=0.5, headwidth=3, headlength=5
                ),
                rotation=45,
            ...
            for index, name in zip([index_top, index_bottom], ["increase", "decrease"]):
                ...
                fig_name = os.path.join(
                    out_path, fname.split(".")[0] + "_{}_dim_{}.png"
                ).format(name, index)
