...
for base_dir, name in zip(directories, names):
    files = glob.glob(os.path.join(base_dir,"**/params_epoch_1000.npz"), recursive=True)
    ...
    for i in range(len(dims)):
        ...
        ax.errorbar(dims[i], val_losses[i], lw=1.2, xerr=errors[i], fmt="o", label=str(betas[i]), c=cmap[i])
    ...
    plt.text(min_dim + 2, max_val - (max_val - min_val) / 1.5,  f"Optimal Model \n n = {min_dim}", fontsize=10, rotation=45)
