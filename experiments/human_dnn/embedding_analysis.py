...
def compare_modalities(weights_human, weights_dnn, duplicates=False, not_sorted=False):
...
def plot_density_scatters(plots_dir, behavior_images, rsm_1, rsm_2, concepts, top_k, mod_1="Human", mod_2="VGG16"):
    ...
    for i, concept in enumerate(concepts.columns):
        ...
        object_similarities = pd.DataFrame(np.c_[tril_1, tril_2, categories], columns=[mod_1, mod_2, 'category'])


        with sns.axes_style('white'):
            g = sns.jointplot(data=object_similarities, x=mod_1, y=mod_2, kind='scatter', 
...
def plot_most_dissim_pairs(plots_dir, rsm_1, rsm_2, mod_1, mod_2, top_k):
    ...
    obj_sims = pd.DataFrame(np.c_[tril_1, tril_2, labels], columns=[mod_1, mod_2, 'labels'])
    with sns.axes_style('ticks'):
        g = sns.jointplot(data=obj_sims, x=mod_1, y=mod_2, hue='labels', palette=dict(enumerate(colors)), height=7, alpha=.6, kind='scatter', legend=False)
        x = obj_sims[mod_1]
        y = obj_sims[mod_2]
        ...
        g.ax_joint.annotate(''.join((r'$\rho$',' = ', str(rho))), (np.min(tril_1), np.max(tril_2)), fontsize=10)
...
def plot_most_dissim_dims(plots_dir, human_dnn_corrs, behavior_images, weights_human, weights_dnn, top_k=4):
    ...
    for dim_idx in tqdm.tqdm(range(n_dims)):
        ...
        for iden, img_indices in zip(["High Human High DNN", "High Human Low Dnn", "High DNN Low Human", "Low Human Low DNN"], 
        ...
        with sns.axes_style('white'):
            ...
            g = sns.jointplot(data=plot_dict, x='human', y='dnn', hue="cluster", legend=False, ax=ax, s=25)
            m, b = np.polyfit(x, y, deg=1)
            # Draw (regression) line of best fit
            g.ax_joint.plot(x, m*x+b, linewidth=2, c='black')
            g.ax_joint.set_xticks([])
...
def find_rank_transformed_dissimilarities(weights_human, weights_dnn, behavior_images, plots_dir, topk=4):
    ...
    for dim_idx in range(n_dim):
        ...
        dim_path = os.path.join(plots_dir, "human_dnn_comparison", str(dim_idx).zfill(2))
        ...
        for (img_indices, identifier) in zip([high_both, high_human_low_dnn, high_dnn_low_human],
...
def plot_mind_machine_corrs(mind_machine_corrs, out_path, **kwargs):
    fig, ax = plt.subplots()
    ax.plot(range(len(mind_machine_corrs)), mind_machine_corrs, **kwargs)
    ax.set_xlabel("Latent dimension")
    ax.set_ylabel("Correlation")
    ax.set_title("Human vs DNN dimensions")
    fig.tight_layout()
    fig.savefig(os.path.join(out_path, "human_dnn_dimension_correlation.png"), dpi=300)


def run_embedding_analysis(human_embedding, dnn_embedding, behavior_images, out_path="./plots"):
    ...
    human_sorted_indices, dnn_sorted_indices, mind_machine_corrs = compare_modalities(human_embedding, dnn_embedding, duplicates=True)
...
if __name__ == '__main__':
    ...
    run_embedding_analysis(args.human_embedding_path, args.dnn_embedding_path, args.feature_path)
