...
def find_topk_images(predictor, features, top_k, latent_dim):
    ...
    top_k_images = top_k_images[:, latent_dim] # take the topk samples for a specific dimension!

    return top_k_images

def search_image_spaces(regression_predictor, dataset, features, top_k=16, window_size=20, stride=1, latent_dim=1, device='cpu'):
    ...
    for i, k in enumerate(top_k_images):    
        ...
        with open(os.path.join(out_path, f'./searchlight_ws_{window_size}_stride_{stride}_{i}.pkl'), 'wb') as f:
...
if __name__ == '__main__':
    ...
    extractor = Extractor(model_name=cfg.model_name, pretrained=True, device=device, source='torchvision')
    dataset = ImageDataset(root=cfg.image_root, out_path='', backend=extractor.backend, transforms=extractor.get_transformations())
    ...
    search_image_spaces(predictor, dataset, features, cfg.top_k, cfg.window_size, cfg.stride, cfg.latent_dim, device)
