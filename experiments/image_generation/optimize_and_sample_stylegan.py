...
class StyleGanGenerator(object):
    ...
    def sample_latents(self):
        ...
        for i in range(self.n_samples):
            ...
            w_latent = gen_utils.get_w_from_seed(
                self.generator,
                1,
                self.device,
                self.truncation,
        ...
        with torch.no_grad():
            for i, batch in enumerate(dataloader):
                ...
                for latent, img in zip(batch, images):
                    ...
                    np.save(
                        os.path.join(self.out_path, "latents", f"{n_samples}.npy"),
                        latent,
                    )
                    n_samples += 1
...
class StyleGanDataset(torch.utils.data.Dataset):
    def __init__(self, base_path="./data/stylegan_dataset", transform=None):
        ...
        assert len(self.latent_paths) == len(
            self.image_paths
        ), "Latent and image paths do not match"
...
class SparseCodesPredictor(object):
    ...
    def __init__(
        ...
        self.dataset = StyleGanDataset(
            base_path="./data/stylegan_dataset", transform=transforms
        )
...
class Optimizer(nn.Module):
    ...
    def optimize_latents(self, generator, comparator):
        ...
        for k, latent in enumerate(sampled_latents):
            ...
            img = (
                gen_utils.w_to_img(generator, optimized_latent, to_np=False)
                .detach()
                .cpu()
                .squeeze(0)
            )
    ...
    def train(self, sampled_latent, generator, comparator):
        ...
        for i in range(self.max_iter):
            ...
            nll = (
                -self.beta * log_code
            )  # push probability mass in softmax towards dimension for which optimization is perfomed (argmax -> self.dim)
            loss = abs_loss + nll
...
if __name__ == "__main__":
    ...
    if args.sample_dataset == "True":
        ...
        generator = StyleGanGenerator(
            out_path="./data/stylegan_dataset",
