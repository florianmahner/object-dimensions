...
def find_topk_latents(
    ...
    dimension_predictor = DimensionPredictor(
        model_name, module_name, device, regression_path
    )
    predictor = SparseCodesPredictor(
        n_samples=n_samples,
        n_dims=dimension_predictor.embedding_dim,
    ...
    predictor.predict_latent(dimension_predictor)
...
class SparseCodesPredictor(object):
    ...
    def predict_latent(self, comparator):
        ...
        for i, (img, _) in enumerate(dataloader):
            ...
            sampled_codes[i * self.batch_size : (i + 1) * self.batch_size] = (
                codes.detach()
            )
